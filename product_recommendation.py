...
def integrate_with_flask_app(app):
    logger.debug("--- Initializing recommendation system for Flask app ---")
    if not os.path.exists('vector_db'):
        logger.debug("Creating vector_db directory")
    ...
    try:
        ...
        # Kiểm tra và tải cơ sở dữ liệu vector
        vector_files = [
        ...
        if all(os.path.exists(f) for f in vector_files):
            ...
            logger.error("Vector database files missing. Please create vector database first.")
            index = None
            df = None
            product_ids = None
    except Exception as e:
    ...
    def get_recommendations(product_id):
        ...
        if index is None or df is None or product_ids is None:
            ...
            return jsonify({
                "error": "Vector database not initialized. Please create vector database first.",
                "status": "error"
            }), 500

        try:
            logger.debug("Parsing limit parameter")
            ...
            logger.debug("Calling get_similar_products_by_id")
            similar_products = get_similar_products_by_id(
                product_id, index, df, product_ids, model, tiki_service, top_k=limit
            )
            ...
            if not similar_products:
                ...
                if len(df) > 0:
                    ...
                    random_products = df.sample(n=min(limit, len(df)), replace=False).to_dict('records')
